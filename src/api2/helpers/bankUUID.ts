...
export const getBankIds = async (req: any): Promise<BankIds> => {
  ...
  if (typeof rawId === 'undefined' || rawId == null || rawId === 'undefined') {
    // this is the deprecated default integer bank id 1
    internalId = 1;
  ...
  if (internalId != null) {
    // NB : usage of internal bank ids for new banks is deprecated since 2025-04-08.
    ...
    if (gRes.rows.length === 1) {
      ...
      throw new BadBankIdException(`Bank id ${internalId} not found.`);
    }
  } else {
