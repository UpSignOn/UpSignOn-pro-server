...
export const createPasswordChallengeV2 = (
  ...
  if (encryptedDataString.startsWith('formatP002-')) {
    ...
    return { pwdChallengeBase64: parts[5], dataFormat: "formatP002", pwdDerivationSaltBase64: parts[4], derivationAlgorithm: parts[1], cpuCost: parseInt(parts[2]), memoryCost: parseInt(parts[3]) };
    ...
    return { pwdChallengeBase64: parts[5], dataFormat: "formatP003", pwdDerivationSaltBase64: parts[4], derivationAlgorithm: parts[1], cpuCost: parseInt(parts[2]), memoryCost: parseInt(parts[3]) };
  }
...
export const checkPasswordChallengeV2 = async (
  ...
  if (!encryptedData.startsWith('formatP002-') && !encryptedData.startsWith('formatP003-')) {
    throw Error("Calling checkPasswordChallengeV2 with a data format that is not formatP002- nor formatP003-");
  }
  // data = 'formatP003-derivationAlgoName-derivationCpuCost-derivationMemoryCost-derivationSalt-passwordChallenge-passwordChallengeExpectedResponse-nonce-cipherText'
...
export const hashPasswordChallengeResultForSecureStorageV2 = (
  ...
  if (!encryptedDataString.startsWith('formatP002-') && !encryptedDataString.startsWith('formatP003-')) {
    throw Error("Calling hashPasswordChallengeResultForSecureStorageV2 with a data format that is not formatP002- nor formatP003-");
  }
  // data = 'formatP003-derivationAlgoName-derivationCpuCost-derivationMemoryCost-derivationSalt-passwordChallenge-passwordChallengeExpectedResponse-nonce-cipherText'
